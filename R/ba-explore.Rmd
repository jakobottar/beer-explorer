...
```{r import2}
ba <- select(ba_raw, -c(brewery_id, review_time, review_profilename))
knitr::kable(head(ba))
```
...
by_brewery <- ba %>%
  ...
  summarize(beers = length(unique(beer_name)), # TODO: Make a json
            beers_id = length(unique(beer_beerid)), # TODO: Make a json
